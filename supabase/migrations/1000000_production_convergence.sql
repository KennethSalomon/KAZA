-- ============================================================================
-- 1000000_production_convergence.sql
-- Production convergence migration for the current KAZA mainline.
--
-- Purpose:
--   Reconcile the known remote-production drift found during the read-only
--   Supabase audit without rewriting migration history and without deleting
--   business data.
--
-- Scope:
--   - converge expenses + expense RPCs
--   - converge landlord dashboard RPCs
--   - converge payment/conversation invariants from 033
--   - converge visit/overdue/push cron definitions
--
-- Important:
--   - This migration does NOT deploy Edge Functions. Edge Functions must be
--     deployed separately from the current main branch after this PR is merged.
--   - Existing business rows are never deleted or rewritten here.
--   - Cron jobs are replaced by name only; no notification/payment data is
--     modified.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. EXPENSES DOMAIN (031 + 032 convergence)
-- ---------------------------------------------------------------------------
...
begin
  if not exists (
    ...
    create type public.expense_category as enum (
      'travaux', 'charges', 'taxes', 'assurance', 'autre'
    );
  end if;
end $$;
...
revoke execute on function public.list_landlord_expenses(date,date,public.expense_category,uuid)
  from public, anon;
grant execute on function public.list_landlord_expenses(date,date,public.expense_category,uuid)
  to authenticated;
...
revoke execute on function public.create_expense(
  public.expense_category,numeric,date,text,uuid,text
) from public, anon;
...
revoke execute on function public.update_expense(
  uuid,public.expense_category,numeric,date,text,uuid,text
) from public, anon;
...
begin
  ...
  select m.month_start,
         em.expected,
         coalesce(cm.collected, 0),
         coalesce(om.overdue, 0),
         coalesce(xm.expenses, 0),
         coalesce(cm.collected, 0) - coalesce(xm.expenses, 0)
    from months m
...
-- ---------------------------------------------------------------------------
-- 2. LANDLORD DASHBOARD CONVERGENCE (030 + app contract)
-- ---------------------------------------------------------------------------
        ...
        from (
          select
            ...
            (
              ...
              - greatest(p.period_start, date_trunc('month', now())::date) + 1
            )::numeric
            / nullif((p.period_end - p.period_start + 1)::numeric, 0)
            as allocated_amount
          from public.payments p
...
-- ---------------------------------------------------------------------------
-- 3. PAYMENT / CONVERSATION CONVERGENCE (033/034)
-- ---------------------------------------------------------------------------
...
-- Protect payment relational identity for all direct UPDATE paths. The
-- existing financial guard remains the authority for amount/period/provider.
...
begin
  if new.lease_id is distinct from old.lease_id
    ...
    raise exception 'Le rattachement d''un paiement est immuable'
      using errcode = '42501';
  end if;
  ...
  if new.amount is distinct from old.amount
    ...
    raise exception 'Les informations financières du paiement sont immuables'
      using errcode = '42501';
  end if;
...
create trigger payments_before_update_financial
  before update on public.payments
  for each row execute function public.payments_guard_financial();
...
begin
  if not v_has_cron or not v_has_net then
    raise notice 'Production cron convergence skipped: pg_cron or pg_net namespace unavailable';
    return;
  end if;

  -- Visit reminders: replace the obsolete Authorization: Bearer job with the
  -- current x-cron-secret contract. The Edge Function itself is deployed
  -- separately from this SQL migration.
  begin
  ...
  -- Overdue reminders: ensure the deployed Edge Function has an active
  -- daily scheduler using the current authentication model.
  begin
  ...
  -- Push emitter is already healthy remotely, but recreate the definition so
  -- its desired configuration is deterministic after convergence.
  begin
...
-- ---------------------------------------------------------------------------
-- 5. NON-DESTRUCTIVE POST-CHECKS
-- ---------------------------------------------------------------------------

do $$
begin
  if to_regclass('public.expenses') is null then
    raise exception 'CONVERGENCE FAILURE: public.expenses missing';
  end if;

  if to_regprocedure('public.landlord_dashboard_stats()') is null then
    raise exception 'CONVERGENCE FAILURE: landlord_dashboard_stats() missing';
  end if;

  if to_regprocedure('public.list_upcoming_due(integer)') is null then
    raise exception 'CONVERGENCE FAILURE: list_upcoming_due(integer) missing';
  end if;
  ...
  if not exists (
    ...
    raise exception 'CONVERGENCE FAILURE: payments_one_active_per_period missing';
  end if;
  ...
  if not exists (
    select 1
     ...
     where schemaname = 'public'
       ...
       and position('is_admin' in lower(coalesce(qual, ''))) > 0
  ) then
    raise exception 'CONVERGENCE FAILURE: conversations_select missing is_admin()';
  end if;

  raise notice 'KAZA production convergence SQL post-checks: OK';
end $$;
