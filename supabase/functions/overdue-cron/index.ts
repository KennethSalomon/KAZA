...
Deno.serve(async (req: Request) => {
  ...
  const supabase = getAdminClient();
  const report = { upcoming: 0, overdue_j1: 0, overdue_j7: 0, emails: 0 };
  ...
  for (const u of (upcoming ?? []) as unknown as UpcomingDue[]) {
    ...
    report.upcoming++;
    const mail = reminderDueIn3Days('Locataire', facts.monthlyRent, u.due_on, APP_URL);
    if (u.tenant_id) {
      const { data: p } = await supabase.from('profiles').select('full_name, email').eq('id', u.tenant_id).maybeSingle();
      if (p?.email && (await sendEmail({ ...mail, to: p.email, html: mail.html.replace('Bonjour Locataire', `Bonjour ${p.full_name}`) }))) {
        report.emails++;
      }
    }
  }
